...
struct ABY : StalysModule {
  ...
  ABY() { config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS); }

  rack::dsp::SchmittTrigger trig[8];
  bool aSelected[8] = {true, true, true, true, true, true, true, true};

  void process(const ProcessArgs &args) override {
    aSelected[0] = trig[0].process(inputs[TRIG1_INPUT].getVoltage())
                       ? !aSelected[0]
                       : aSelected[0];
    aSelected[1] = trig[1].process(inputs[TRIG2_INPUT].getVoltage())
                       ? !aSelected[1]
                       : aSelected[1];
    aSelected[2] = trig[2].process(inputs[TRIG3_INPUT].getVoltage())
                       ? !aSelected[2]
                       : aSelected[2];
    aSelected[3] = trig[3].process(inputs[TRIG4_INPUT].getVoltage())
                       ? !aSelected[3]
                       : aSelected[3];
    aSelected[4] = trig[4].process(inputs[TRIG5_INPUT].getVoltage())
                       ? !aSelected[4]
                       : aSelected[4];
    aSelected[5] = trig[5].process(inputs[TRIG6_INPUT].getVoltage())
                       ? !aSelected[5]
                       : aSelected[5];
    aSelected[6] = trig[6].process(inputs[TRIG7_INPUT].getVoltage())
                       ? !aSelected[6]
                       : aSelected[6];
    aSelected[7] = trig[7].process(inputs[TRIG8_INPUT].getVoltage())
                       ? !aSelected[7]
                       : aSelected[7];

    outputs[OUT1_OUTPUT].setVoltage(aSelected[0]
                                        ? inputs[A1_INPUT].getVoltage()
                                        : inputs[B1_INPUT].getVoltage());
    outputs[OUT2_OUTPUT].setVoltage(aSelected[1]
                                        ? inputs[A2_INPUT].getVoltage()
                                        : inputs[B2_INPUT].getVoltage());
    outputs[OUT3_OUTPUT].setVoltage(aSelected[2]
                                        ? inputs[A3_INPUT].getVoltage()
                                        : inputs[B3_INPUT].getVoltage());
    outputs[OUT4_OUTPUT].setVoltage(aSelected[3]
                                        ? inputs[A4_INPUT].getVoltage()
                                        : inputs[B4_INPUT].getVoltage());
    outputs[OUT5_OUTPUT].setVoltage(aSelected[4]
                                        ? inputs[A5_INPUT].getVoltage()
                                        : inputs[B5_INPUT].getVoltage());
    outputs[OUT6_OUTPUT].setVoltage(aSelected[5]
                                        ? inputs[A6_INPUT].getVoltage()
                                        : inputs[B6_INPUT].getVoltage());
    outputs[OUT7_OUTPUT].setVoltage(aSelected[6]
                                        ? inputs[A7_INPUT].getVoltage()
                                        : inputs[B7_INPUT].getVoltage());
    outputs[OUT8_OUTPUT].setVoltage(aSelected[7]
                                        ? inputs[A8_INPUT].getVoltage()
                                        : inputs[B8_INPUT].getVoltage());
  }
};
