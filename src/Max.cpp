...
struct Max : StalysModule {
  ...
  Max() { config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS); }
  ...
  void process(const ProcessArgs &args) override {
    ...
    const float outs[8] = {std::max(std::max(ins[0], ins[1]), ins[2]),
    ...
    const float out2 = outputs[OUT1_OUTPUT].isConnected()
                           ? outs[1]
                           : std::max(outs[0], outs[1]);
    const float out3 =
        outputs[OUT2_OUTPUT].isConnected() ? outs[2] : std::max(out2, outs[2]);
    const float out4 =
        outputs[OUT3_OUTPUT].isConnected() ? outs[3] : std::max(out3, outs[3]);
    const float out5 =
        outputs[OUT4_OUTPUT].isConnected() ? outs[4] : std::max(out4, outs[4]);
    const float out6 =
        outputs[OUT5_OUTPUT].isConnected() ? outs[5] : std::max(out5, outs[5]);
    const float out7 =
        outputs[OUT6_OUTPUT].isConnected() ? outs[6] : std::max(out6, outs[6]);
    const float out8 =
        outputs[OUT7_OUTPUT].isConnected() ? outs[7] : std::max(out7, outs[7]);
