...
struct Avg : StalysModule {
  ...
  Avg() { config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS); }
  ...
  void process(const ProcessArgs &args) override {
    const float in[24] = {
        ...
        inputs[IN17_INPUT].getVoltage(), inputs[IN18_INPUT].getVoltage(),
        inputs[IN19_INPUT].getVoltage(), inputs[IN20_INPUT].getVoltage(),
    ...
    const bool inConnected[24] = {
        ...
        inputs[IN17_INPUT].isConnected(), inputs[IN18_INPUT].isConnected(),
        inputs[IN19_INPUT].isConnected(), inputs[IN20_INPUT].isConnected(),
        inputs[IN21_INPUT].isConnected(), inputs[IN22_INPUT].isConnected(),
        inputs[IN23_INPUT].isConnected(), inputs[IN24_INPUT].isConnected()};
    const bool outConnected[8] = {
        outputs[OUT1_OUTPUT].isConnected(), outputs[OUT2_OUTPUT].isConnected(),
        outputs[OUT3_OUTPUT].isConnected(), outputs[OUT4_OUTPUT].isConnected(),
        outputs[OUT5_OUTPUT].isConnected(), outputs[OUT6_OUTPUT].isConnected(),
        outputs[OUT7_OUTPUT].isConnected(), outputs[OUT8_OUTPUT].isConnected()};
